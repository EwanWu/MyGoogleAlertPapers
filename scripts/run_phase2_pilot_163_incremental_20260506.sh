...
STATE="$ROOT/data/task_state/phase2_pilot_163_incremental_20260506.json"
LOG="$ROOT/data/logs/phase2_pilot_163_incremental_20260506.log"
REVIEW_EXPORT="$ROOT/data/exports/review_queue_phase2_pilot_20260506.jsonl"
...
with sqlite3.connect(db_path) as conn:
    ...
    dedup_row = conn.execute("SELECT notes FROM cost_event WHERE stage = 'dedup_candidates' ORDER BY id DESC LIMIT 1").fetchone()
    metrics['latest_dedup_note'] = dedup_row[0] if dedup_row else None
...
on_error() {
  ...
  update_state "failed" "$STEP" "inspect $LOG and the state file before retrying the pilot" || true
...
echo "[$(date --iso-8601=seconds)] Phase 2 pilot start"
echo "ROOT=$ROOT"
...
STEP="reporting"
update_state "running" "$STEP" "phase2 pilot complete"
...
STEP="phase2-pilot-complete"
update_state "completed" "$STEP" "decide whether to promote to regular 5000-candidate chunks"
echo "[$(date --iso-8601=seconds)] Phase 2 pilot completed"
