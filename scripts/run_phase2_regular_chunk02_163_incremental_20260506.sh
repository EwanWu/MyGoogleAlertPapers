...
LOG="$ROOT/data/logs/phase2_regular_chunk02_163_incremental_20260506.log"
...
on_error() {
  ...
  update_state "failed" "$STEP" "inspect $LOG and the state file before retrying regular chunk02" || true
...
echo "[$(date --iso-8601=seconds)] Phase 2 regular chunk02 start"
echo "ROOT=$ROOT"
...
STEP="reporting"
update_state "running" "$STEP" "phase2 regular chunk02 complete"
...
STEP="phase2-regular-chunk02-complete"
update_state "completed" "$STEP" "decide whether to continue regular chunk03 at the same size"
echo "[$(date --iso-8601=seconds)] Phase 2 regular chunk02 completed"
