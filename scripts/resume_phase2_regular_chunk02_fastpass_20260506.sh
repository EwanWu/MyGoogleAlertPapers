...
STATE="$ROOT/data/task_state/phase2_regular_chunk02_163_incremental_20260506.json"
LOG="$ROOT/data/logs/phase2_regular_chunk02_fastpass_resume_20260506.log"
REVIEW_EXPORT="$ROOT/data/exports/review_queue_phase2_regular_chunk02_20260506.jsonl"
...
with sqlite3.connect(db_path) as conn:
    ...
    row = conn.execute("SELECT run_id, status, processed_count, notes FROM batch_run WHERE stage='enrich_candidates' ORDER BY id DESC LIMIT 1").fetchone()
    if row:
        metrics['latest_enrich_run_id'] = row[0]
        metrics['latest_enrich_status'] = row[1]
        metrics['latest_enrich_processed_count'] = row[2]
        try:
            metrics['latest_enrich_notes'] = json.loads(row[3]) if row[3] else None
        except Exception:
            metrics['latest_enrich_notes'] = row[3]
...
on_error() {
  ...
  update_state "failed" "$STEP" "inspect $LOG and the state file before retrying chunk02 fast-pass resume" || true
...
echo "[$(date --iso-8601=seconds)] Phase 2 regular chunk02 fast-pass resume start"
echo "ROOT=$ROOT"
...
STEP="reporting"
update_state "running" "$STEP" "phase2 regular chunk02 fast-pass complete"
...
STEP="phase2-regular-chunk02-fastpass-complete"
update_state "completed" "$STEP" "decide whether to continue chunk03 regular fast-pass or run a separate slow title-lane residual sweep"
echo "[$(date --iso-8601=seconds)] Phase 2 regular chunk02 fast-pass resume completed"
